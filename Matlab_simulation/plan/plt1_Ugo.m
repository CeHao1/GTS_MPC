...
%         plot(constraints(1,:),'b')
        plot(xout(i,:),'r')
        hold off   
        axis([1,length(xout),min(xout(i,:))-1,max(xout(i,:))+1])
    else
        plot(xout(i,:))
    end
...
kap_u=get_center_kap(X_u,Y_u);

figure
hold on
plot(s_pos,pos(5,:),'b')
h2=plot(s_pos,kap,'k','linewidth',2);
...
figure
hold on
h1=plot(s_u,Vx_u,'b','linewidth',2);
h2=plot(s_pos,Vx,'r','linewidth',2);

hold off
legend([h1,h2],'\fontsize {15}LMPC','\fontsize {15}IPC')
title('Vx, Lap time of IPC is 6.16s','fontsize',15)
