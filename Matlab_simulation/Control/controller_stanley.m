function [sys,x0,str,ts,simStateCompliance] = controller_stanley(t,x,u,flag,reference)
...
idx1=(possible_points>max(reference(:,1))); % exceed 
idx2=(possible_points<min(reference(:,1))); % below
possible_points(idx1)=possible_points(idx1)-max(reference(:,1));
possible_points(idx2)=possible_points(idx2)+max(reference(:,1));
...
ks=2.0;
lamb=0.5;
steer=-(ks*epsi+atan2(lamb*ey,u(4)));

steer_limit=6/180*pi;
if abs(steer)>steer_limit
    sys(2)=steer_limit*sign(steer);
else
    sys(2)=steer;
end
...
idealVx=spline(reference(:,1),reference(:,4),s);
fb=idealVx-u(4);
if abs(fb)<gap
    sys(1)=maxa*fb/gap;
else
